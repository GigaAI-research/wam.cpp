...
float FlowMatchEulerSchedule::delta_sigma(std::size_t step) const {
    if (sigmas.size() != timesteps.size() + 1 || step >= timesteps.size()) {
        invalid("flow-match scheduler step is out of range", "step",
                std::to_string(step));
    }
...
FlowMatchEulerSchedule make_flow_match_euler_schedule(int steps, float shift) {
    if (steps <= 0) {
        invalid("flow-match step count must be positive", "steps",
                std::to_string(steps));
    }
    if (!std::isfinite(shift) || shift <= 0.0F) {
        invalid("flow-match shift must be finite and positive", "shift",
                std::to_string(shift));
    }
    ...
    const float shifted_sigma_min =
        shift * training_sigma /
        (1.0F + (shift - 1.0F) * training_sigma);
    const double minimum_timestep =
        static_cast<double>(shifted_sigma_min) * 1000.0;
    for (int step = 0; step < steps; ++step) {
        ...
        schedule.sigmas[static_cast<std::size_t>(step)] =
...
void flow_match_euler_step(std::vector<float> & action,
    ...
    if (!std::isfinite(delta_sigma)) {
        invalid("flow-match delta sigma must be finite", "delta_sigma",
                "non-finite");
    }
