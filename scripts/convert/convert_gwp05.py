...
def _add_metadata(
    ...
    *,
    ...
    action_chunk: int,
    inference_steps: int,
    policy_spec: Mapping[str, object],
    ...
    add_policy_spec_metadata(writer, policy_spec)
    ...
    writer.add_string(KV("norm_mode"), policy_spec["normalization"]["state"]["kind"])
...
def main() -> int:
    ...
    parser.add_argument(
        "--policy-spec", type=Path, required=True,
        help="versioned PolicySpec profile JSON embedded into the GGUF",
    )
    ...
    try:
        policy_spec = load_policy_profile(
            args.policy_spec.resolve(),
            model_dim=int(transformer_cfg["in_action_channels"]),
    ...
    result = {
        ...
        "inputs": {
            ...
            "policy_spec": str(args.policy_spec.resolve()),
        },
